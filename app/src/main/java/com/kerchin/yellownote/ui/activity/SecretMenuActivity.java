...
//    ToggleButton mSecretMenuPatternToggle;
    ...
    protected void initializeData(Bundle savedInstanceState) {
        ...
        ThreadPool.getInstance().execute(new Runnable() {
            ...
            public void run() {
                try {
                    String str = SecretService.getPatternStr(SampleApplicationLike.user);
...
//        mSecretMenuPatternToggle.setChecked(!mSecretMenuPatternToggle.isChecked());
        ThreadPool.getInstance().execute(new Runnable() {
            ...
            public void run() {
                try {
                    final String str = SecretService.getPatternStr(SampleApplicationLike.user);
        ...
        if (requestCode == requestForPattern && resultCode == RESULT_OK) {//创建成功
            ThreadPool.getInstance().execute(new Runnable() {
                ...
                public void run() {
                    try {
                        if (!hasPattern) {//开启手势
                            SecretService.setPatternStr(SampleApplicationLike.user, patternFromOthers);
